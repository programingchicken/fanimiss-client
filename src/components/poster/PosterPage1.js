...
import FormDetailPoster from '../temps/formDetailPoster';
...
class PosterPage1 extends Component {
  //org state
  state = {
    imgs: "",
    switchs: "",
      ...
      for (const obj of arr) {
        if (obj.id === Number(id[2])) {
          ...
          this.setState({
            ...
            size: obj.size,
            display: obj.display,
            priceSwitchs: obj.display === 3 ? [obj.listA[0].price,obj.listA[1].price]:[obj.list[0].price,obj.list[1].price],
            switchs: obj.display === 3 ? [obj.listA[0].name,obj.listA[1].name]:[obj.list[0].name,obj.list[1].name],
            imgs: obj.display === 3 ?[obj.listA[0].imgs,obj.listA[1].imgs] : [obj.list[0].imgs,obj.list[1].imgs]
          });
      ...
      <div style={{ backgroundColor: "#001220"}} className="d-flex justify-content-center align-items-center flex-wrap">
        ...
        <div className="sho cont rounded-5 d-flex justify-content-center align-items-center flex-wrap">
             ...
             <div className="sho inn rounded-5 d-flex justify-content-left align-items-top flex-wrap">
                ...
                <span className="titleName">{name} {size}</span>
...
this.props.history.push("/posters/small");
      ...
      const {name,stars,id,img,token, quantity,size} = this.state
let item = {name: `${newName}`,stars:stars,price: price,id: id2,img,token, quantity, nameArr: [{nameC: newAName,nameD: newBName, quantityC: 1 } ]}
...
if (arr.length !== 0) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (arr[i].id === item.id) {
      ...
      console.log(newA)
      for (let j = 0; j < arr[i].nameArr.length; j++) {
        newNameArr.set(arr[i].nameArr[j].nameC+arr[i].nameArr[j].nameD, {nameC: arr[i].nameArr[j].nameC,nameD: arr[i].nameArr[j].nameD, quantityC:arr[i].nameArr[j].quantityC})
        console.log(newNameArr)
        // if (newNameArr.has(item.nameArr[0].nameC)) {
        if ( arr[i].nameArr[j].nameC+arr[i].nameArr[j].nameD === item.nameArr[0].nameC+ item.nameArr[0].nameD) {
          item.nameArr[0].quantityC += arr[i].nameArr[j].quantityC
console.log('add', item.nameArr[0].quantityC)
...
newNameArr.set(item.nameArr[0].nameC+ item.nameArr[0].nameD, {nameC:item.nameArr[0].nameC,nameD:item.nameArr[0].nameD, quantityC: item.nameArr[0].quantityC})
console.log(newNameArr)
item.nameArr = [...other,item.nameArr[0]]
console.log(item.nameArr)
          // newNameArr.delete(item.nameArr.nameC)
      ...
      newNameArr.set(item.nameArr[0].nameC+ item.nameArr[0].nameD,{nameC:item.nameArr[0].nameC,nameD:item.nameArr[0].nameD, quantityC: item.nameArr[0].quantityC})
      console.log(newNameArr.get(newAName+newBName))
      console.log(other)
      item.nameArr = [...other, newNameArr.get(newAName+newBName)]
      console.log(item.nameArr)
      console.log(newNameArr)
      ...
      newNameArr.set(item.nameArr[0].nameC+ item.nameArr[0].nameD,{nameC:item.nameArr[0].nameC,nameD:item.nameArr[0].nameD, quantityC: item.nameArr[0].quantityC})
      item.nameArr = newNameArr.get(item.nameArr[0].nameC+item.nameArr[0].nameC)
      newarr.push(item)
    } else {
      newarr.push(arr[i])
    }
  ...
  newNameArr.set(item.nameArr[0].nameC+item.nameArr[0].nameD,{nameC: item.nameArr[0].nameC,nameD: item.nameArr[0].nameD, quantityC: item.nameArr[0].quantityC})
  console.log(newNameArr)
  item.nameArr = [newNameArr.get(item.nameArr[0].nameC+item.nameArr[0].nameD)]
  console.log(item)
newarr.push(item)
}
...
this.props.history.push("/posters/small");
  ...
  handelReviews = (size, size2, price) => {
    const {name, switchs, imgs,priceSwitchs} = this.state
    document.getElementById('pageImg').src = size ? imgs[1] : imgs[0]
    document.getElementsByClassName('titleName')[0].innerHTML = `${name} ${size ? switchs[1] : switchs[0]} `
document.getElementsByClassName('titlePrice')[0].innerHTML = `${size ? priceSwitchs[1] : priceSwitchs[0]} `
